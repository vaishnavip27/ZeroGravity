  ...
  return (
    <div className=" mx-auto mt-6">
      {faqs.map((faq: FaqItem, index: number) => (
        <div
          key={index}
          className={`border-b border-[#FE6A00]  mb-8  ${index===selectedQuestion?"pb-4":"pb-1"}`}
          onClick={() => toggleQuestion(index)}
        >
          <div className="flex items-center justify-between">
            <h3 className=" font-semibold text-[#b0b0b0] text-2xl">
              {faq.question}
            </h3>
            <span
              className={`${
          ...
          >
            <p className="text-[#7d7d7d] font-universolight text-lg">{faq.answer}</p>
          </div>
        </div>
      ))}
    </div>
